...
. This script will produce the figure below. 
...
To visualise the eigenfunctions of the Laplace-Beltrami operator, you can run the script *visualise\_eigenfunctions.py*. This script project the eigenfunctions onto the mesh with no holes which, in turn, corresponds to the mesh that approximates the unit sphere. The files that are generated by this script are stored in the Output folder, and they can be looked at in ParaView.


If you wish to save a particular initial condition to a file, you can run the script "*fix\_initial\_condition.py*". After this, you can alter the sixth or, if you will, second last argument on the lines 72 and 75 in the script "*launch\_simulations\_Schnakenberg\_sphere\_with\_holes.py*" from "*True*" to "*False*". In this case, the initial condition on the mesh without any holes will be fixed, and if you conduct simulations on meshes with a hole at the south pole the saved initial condition will be projected onto these meshes. 


Lastly, it is worth mentioning that currently it is only the initial condition and the concentration profile at the final time point corresponding to t=50 that are saved when the simulations are launched. This is because of the fact that the generated data files take up a lot of memory, and if you save the concentration profile at more intermediate time points for all simulations it is a high risk that you use up all your memory on your disk. However, if you wish to look at, say, a single simulation with more intermediate time points, you must first change the launch-script so that you only run a single simulation. Thereafter, you can uncomment lines 582 and 584 in the script "*toolbox\_FEM\_simulations\_Schnakenberg\_sphere\_with\_holes.py*". By uncommenting these lines, you will save the concentration profile every time the time t has increased by 0.5. 
