...
def residual_Schnakenberg_sphere_with_holes(parameters,phi_1,phi_2,U_prev,U_curr,dx_list,activation_parameters,d,k):
    ...
    # Define the modified reaction terms if we have a hole
    if len(dx_list)>1:         # Discretisation 1  
    ...
    residual_form = mass_form + k*(gamma*reaction_form + stiffness_form)
...
def FEMFD_simulation_Schnakenberg_sphere_with_holes(num_holes,parameters,steady_states,numerical_parameters,activation_parameters,cell_growth):
    ...
    # Calculate matrices and vectors from the VF
    mass_matrix, stiffness_matrix, reaction_matrix, mass_load_vector_rhs, stiffness_load_vector_rhs, reaction_load_vector_rhs = VF_and_FEM_Schnakenberg_sphere_with_holes(parameters,u,v,phi_1,phi_2,U_prev,dx_list,mesh,activation_parameters)
    ...
    #dt = 1e-2 # Doesn't work for any of the 3 disc-schemes T=1.0
    #dt = 5e-3 # Doesn't work for any of heurisitcs and 1-SBEM T=1.0.
    #dt = 1e-3 # Does not work for 1-SBEM T=1.5
    #dt = 5e-4 # Doesn't work for any of heurisitcs and 1-SBEM. Fail at T=1.3.
    #dt = 6.2e-4 # Maximum time-step for 1-SBEM according to Madzvamuse
    #dt = 1e-4 # Doesn't work for 1-SBEM T=1.5
    #dt = 5e-5 # Doesn't work for 1-SBEM T=1.5
    #dt = 1e-5 # Doesn't work for 1-SBEM T=1.5
    #dt = 5e-6 # Doesn't work for 1-SBEM T=1.5
    ...
    TOL_tadapt = 1e-19 # THIS SEEMS TO WORK WITH sigma=1e-4
    #TOL_tadapt = 1.0
    #dt_max = 1e-5 # An upper limit on the maximum time step
    dt_max = 5e-4 # THIS SEEMS TO WORK WITH sigma=1e-4
    #dt_max=T/10000
    #--------------------------------------------------------------
    ...
    while t < T:
        ...
        # Update the matrices and vectors for the LES    
        mass_matrix, stiffness_matrix, reaction_matrix, mass_load_vector_rhs, stiffness_load_vector_rhs, reaction_load_vector_rhs = VF_and_FEM_Schnakenberg_sphere_with_holes(parameters_as_constants,u,v,phi_1,phi_2,U_prev,dx_list,mesh,activation_parameters)
        ...
        #print("\t\tl2_norm_of_R = ", norm(R, 'l2'))
        #print("\t\tdt = ", dt)

        # Compute relative error
        #rel_err = norm(U_curr.vector() - U_prev.vector(), 'l2')/norm(U_curr.vector(), 'l2')
        ...
        # Save the solution (every whatever iteration)
        if t_it % 100 == 0:
            ...
            #rel_err = norm(U_curr.vector() - U_prev.vector(), 'l2')/norm(U_curr.vector(), 'l2')
            #print("\t\tRelative error = ", rel_err)
            #if l2_norm_R > 1e-2 or rel_err > 1e-2:
            if U_curr.vector().min() < 0:
